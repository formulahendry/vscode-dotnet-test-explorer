...
import { IDisposable, toDisposable, combinedDisposable } from './lifecycle';
	...
	const emitter = new Emitter<O>({
		onFirstListenerAdd() {
			subscription = event(cur => {
				...
				(<any>clearTimeout)(handle);
				handle = (<any>setTimeout)(() => {
					let _output = output;
					output = undefined;
...
export function echo<T>(event: Event<T>, nextTick = false, buffer: T[] = []): Event<T> {
	...
	const emitter = new Emitter<T>({
		onListenerDidAdd(emitter, listener, thisArgs?) {
			if (nextTick) {
				(<any>setTimeout)(() => flush(listener, thisArgs));
			} else {
