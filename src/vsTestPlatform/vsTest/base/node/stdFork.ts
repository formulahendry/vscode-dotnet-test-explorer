...
export function fork(modulePath: string, args: string[], options: IForkOpts, callback: (error: any, cp: cp.ChildProcess) => void): void {
	...
	// Create the process
	let bootstrapperPath = (uri.parse(path.join(path.dirname(module.filename), './stdForkStart.js')).fsPath);

	//bootstrapperPath = "C:\\Git\\test-adapter-protocol-client\\dist\\src\\base\\node\\stdForkStart.js";
//bootstrapperPath = modulePath;
	...
	childProcess = cp.fork(bootstrapperPath, [modulePath].concat(args), {
		...
		cwd: options.cwd,
		env: newEnv,
		execArgv: ["--inspect=1234"/*, "--debug-brk"*/]/* options.execArgv*/
	});

	/*childProcess = cp.spawn("node", [bootstrapperPath], {
		//silent: true,
		cwd: options.cwd,
		env: newEnv,
